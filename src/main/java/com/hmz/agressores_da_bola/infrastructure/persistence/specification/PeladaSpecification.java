package com.hmz.agressores_da_bola.infrastructure.persistence.specification;
 ...
 * restringe nada. Assim {@link #de(PeladaFiltro)} apenas empilha os
 * critérios, sem uma cascata de ifs.
 */
public final class PeladaSpecification {
    ...
    /**
     * Empilha todos os critérios do filtro; os não informados não restringem nada.
     */
    public static Specification<Pelada> de(PeladaFiltro filtro) {
        return Specification.allOf(
                comStatus(filtro.status()),
                comTipoCampo(filtro.tipoCampo()),
                naCidade(filtro.cidade()),
                aPartirDe(filtro.dataInicial()),
                ate(filtro.dataFinal()),
                doOrganizador(filtro.organizadorId()),
                comParticipante(filtro.participanteId())
        );
    }
